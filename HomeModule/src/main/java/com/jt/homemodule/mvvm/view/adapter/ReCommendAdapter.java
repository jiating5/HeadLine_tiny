...
public class ReCommentAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ReCommentAdapter(Context context, List<RecommendNewsBean> list) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == 0){
            ...
            return  new MyHodler3(LayoutInflater.from(context).inflate(R.layout.recommend_item3,parent,false));
        }
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (getItemViewType(position) == 0){
            ...
            Glide.with(context).load(list.get(position).getMainimgurl()).into(hodler3.imageView);
        }
    }
